...
describe("item ratings", () => {
  ...
  describe("[PUT] /api/items/:item_id/item-ratings/:item_rating_id", () => {
    it("[1] requests without a token are rejected with right status and message", async () => {
      const res = await request(server).put("/api/items/1/item-ratings/1");
      expect(res.body.message).toMatch(/token required/i);
    });
    it("[2] requests with invalid token are rejected with right status and message", async () => {
      const res = await request(server)
        .put("/api/items/1/item-ratings/1")
        .set("Authorization", "qwerty");
      expect(res.body.message).toMatch(/token invalid/i);
    });
    it("[3] successfully edits an item rating object", async () => {
      const loginRes = await request(server)
        .post("/api/auth/login")
        .send({ username: "jeff", password: "1234" });
      const putRes = await request(server)
      ...
      const itemRatingUpdated = await db("item_ratings")
        .where("item_rating_id", putRes.body.rating.item_rating_id)
        .first();
      expect(itemRatingUpdated).toMatchObject({
        user_id: 1,
        item_id: 1,
        item_rating: 5,
      });
    ...
    it("[4] responds with correct status and message on successful edit", async () => {
      ...
      expect(putRes.body.message).toMatch(/item rating .* updated/i);
      expect(putRes.status).toBe(200);
    });
    it("[5] responds with updated item rating on successful edit", async () => {
      const loginRes = await request(server)
        .post("/api/auth/login")
        .send({ username: "jeff", password: "1234" });
      const putRes = await request(server)
    ...
    it("[6] fails to update when a user doesn't own the item rating", async () => {
      const loginRes = await request(server)
        .post("/api/auth/login")
        .send({ username: "clara", password: "1234" });
      const putRes = await request(server)
    ...
    it("[7] responds with correct status and message on attempt to edit a non-existent item rating", async () => {
      const loginRes = await request(server)
        .post("/api/auth/login")
        .send({ username: "jeff", password: "1234" });
      const res = await request(server)
        .put("/api/items/1/item-ratings/100")
        .set("Authorization", loginRes.body.token)
        .send({
          user_id: 1,
          item_id: 1,
    ...
    it("[8] cannot edit an item rating to a different user_id", async () => {
      const loginRes = await request(server)
        .post("/api/auth/login")
        .send({ username: "jeff", password: "1234" });
      const res = await request(server)
